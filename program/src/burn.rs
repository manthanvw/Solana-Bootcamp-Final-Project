...
pub fn burn(
    ...
    // Ensure that the owner of the NFT matches the burn_owner

    // Check if the NFT is in the account

    // Implement additional checks or business logic here if needed

    // Burn the NFT using the SPL token program
    csl_spl_token::src::cpi::burn(for_burn, Default::default())?;

    // Update the Gem metadata to indicate the NFT is burned
    gem.data.assoc_account = None; // Set assoc_account to None to indicate burned
