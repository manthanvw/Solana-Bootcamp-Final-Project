 ...
 *
 * Data:
 * - new_color: {@link string} 
 * - new_rarity: {@link string} 
 * - new_description: {@link string} 
 * - modify_owner: {@link PublicKey} 
 */
    Modify = 3,
}
    ...
    const data = serialize(
        {
            struct: {
                ...
                mint_owner: { array: { type: "u8", len: 32 } },
            },
        },
        {
            ...
            mint_owner: args.mintOwner.toBytes(),
        }
    );
...
export const transferSendAndConfirm = async (
    args: Omit<TransferArgs, "feePayer" |"funding" |"authority" | "newOwner"> & { 
        signers: { feePayer: Keypair,  funding: Keypair,  authority: Keypair, newOwner: Keypair  }
    ...
    trx.add(transfer({
        ...
        newOwner: args.signers.authority.publicKey,
    }));
    ...
    const data = serialize(
        {
            struct: {
                id: "u8",
                burn_owner: { array: { type: "u8", len: 32 } },
            },
        },
        {
            id: NftInstruction.Burn,
            burn_owner: args.burnOwner.toBytes(),
        }
    );
...
export const burnSendAndConfirm = async (
    args: Omit<BurnArgs, "feePayer" |"owner"| "burnOwner"> & { 
        signers: { feePayer: Keypair,  owner: Keypair, burnOwner:Keypair }
 }
): Promise<TransactionSignature> => {
    const trx = new Transaction();
    ...
    trx.add(burn({
        ...
        feePayer: args.signers.feePayer.publicKey,
        owner: args.signers.owner.publicKey,
        burnOwner: args.signers.burnOwner.publicKey,
    }));

    return await sendAndConfirmTransaction(
        _connection,
        trx,
        [args.signers.feePayer, args.signers.owner, args.signers.burnOwner ]
    );
};
 ...
 * - modify_owner: {@link PublicKey} 
 */
export const modify = (args: ModifyArgs): TransactionInstruction => {
    const data = serialize(
        {
            struct: {
                id: "u8",
                ...
                modify_owner: { array: { type: "u8", len: 32 } },
            },
        },
        {
            id: NftInstruction.Modify,
            new_color: args.newColor,
            new_rarity: args.newRarity,
            new_description: args.newDescription,
            modify_owner: args.modifyOwner.toBytes(),
        }
    );
 ...
 * - modify_owner: {@link PublicKey} 
 */
export const modifySendAndConfirm = async (
    args: Omit<ModifyArgs, "feePayer" |"owner"> & { 
